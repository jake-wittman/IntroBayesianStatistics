...
mod1 <- stan_model(here::here("lectures/code/car.stan"))

mod1_out <- sampling(mod1,
                     data = list(
                       y = data$y,
                       E = data$E,
                       x = data$x,
...
print(
  mod1_out,
  pars = c('beta0', 'beta1', 'phi', 'eta', 'mu', 'alpha'),
  digits = 3,
  probs = c(.025, .5, .975)
)
sample.para <-
  extract(mod1_out, pars = c('beta0', 'beta1', 'phi', 'eta', 'mu', 'alpha'))
sample.para
